...
/// struct for typed errors of method [`styles_items_item_id_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum StylesItemsItemIdGetError {
...
pub async fn styles_items_item_id_get(configuration: &configuration::Configuration, item_id: &str, token: Option<&str>, f: Option<&str>) -> Result<serde_json::Value, Error<StylesItemsItemIdGetError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/styles/items/{item_id}", local_var_configuration.base_path, item_id=crate::apis::urlencode(item_id));
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<StylesItemsItemIdGetError> = serde_json::from_str(&local_var_content).ok();
